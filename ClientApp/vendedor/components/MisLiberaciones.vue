  ...
  <div class="mis-liberaciones">
    <div class="card border-0 shadow-sm rounded-3">
      ...
      <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center px-4 pt-4 pb-3">
        ...
        <button class="btn btn-outline-success btn-sm" @click="cargarLiberaciones(1)"
                :disabled="loading">
            ...
            <table class="table mb-0" style="font-size:.875rem;">
              ...
              <tbody>
                <tr v-for="lib in liberaciones" :key="lib.id"
                  ...
                  <!-- Monto venta -->
                  <td class="border-0 py-3 text-end small">{{ formatMoneda(lib.montoVenta) }}</td>

                  <!-- Comisión -->
                  <td class="border-0 py-3 text-end small text-danger">
                    -{{ formatMoneda(lib.montoComision) }}
                  ...
                  <td class="border-0 py-3 text-end fw-bold text-success">
                    {{ formatMoneda(lib.montoLiberado) }}
                  </td>

                  <!-- Estado -->
                  <td class="border-0 py-3 text-center">
                    <span class="badge rounded-pill" style="font-size:.67rem;"
                          :class="lib.estado === 'Liberado' ? 'bg-success' : lib.estado === 'Pendiente' ? 'bg-warning text-dark' : 'bg-secondary'">
                      {{ lib.estado === 'Liberado' ? '✅ Acreditado'
                       : lib.estado === 'Pendiente' ? '⏳ Pendiente'
                       : lib.estado ?? '—' }}
                    </span>
...
async function cargarLiberaciones(pagina = 1) {
  try {
    ...
    // Endpoint: GET /api/LiberacionApi/vendedor/{vendedorId}
    ...
    kpis.value = {
      totalCobrado:    todas.filter(l => l.estado === 'Liberado').reduce((s, l) => s + (l.montoLiberado ?? 0), 0),
      cobradoMes:      todas.filter(l => l.estado === 'Liberado' && new Date(l.fechaLiberacion).getMonth() === ahora.getMonth()).reduce((s, l) => s + (l.montoLiberado ?? 0), 0),
      pendiente:       todas.filter(l => l.estado === 'Pendiente').reduce((s, l) => s + (l.montoLiberado ?? 0), 0),
      totalComisiones: todas.filter(l => l.estado === 'Liberado').reduce((s, l) => s + (l.montoComision ?? 0), 0),
      totalVentas:     todas.reduce((s, l) => s + (l.montoVenta ?? 0), 0),
